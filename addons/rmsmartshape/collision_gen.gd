...
static func simple_offset_open_polygon_miter(points: PackedVector2Array, offset: float) -> PackedVector2Array:
	...
	var a := points[0]
	var b := points[1]
	var ab_orth := (b - a).orthogonal().normalized()
	...
	for i in range(1, points.size() - 1):
		...
		if clockwise and miter_denom < miter_threshold:
			...
			# We consider the triangle `b2`, `top` and `P`.
			# `b2` and `top` are known as well as their directions (`ab`, `d_orth`) towards `P`.
			...
			# 2) Resolve for `t` using Cramer's rule
			var det := dnorm.x * ab_orth.y - dnorm.y * ab_orth.x
			var t := (rhs.x * d_orth.y - rhs.y * d_orth.x) / det
			var cap_half_length := t * d_orth
