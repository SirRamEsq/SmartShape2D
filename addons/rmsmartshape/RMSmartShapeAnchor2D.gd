...
@export (NodePath) var monitored_shape : set = _set_monitored_shape
@export (int) var track_control_point : set = _set_track_control_point
@export (float) var normal_length = 100.0 : set = _set_normal_length
@export (float, -1.0, 1.0) var control_point_offset : set = _set_control_point_offset
@export (float, -3.14159, 3.14159) var rotation_offset = 0 : set = _set_rotation_offset
@export (bool) var copy_scale = false : set = _set_copy_scale
...
func _set_monitored_shape(value):
	if value == null and monitored_shape != null:
		if has_node(monitored_shape):
			get_node(monitored_shape).disconnect("points_modified",Callable(self,"_handle_point_change"))
			get_node(monitored_shape).disconnect("tree_exiting",Callable(self,"_monitored_node_leaving"))
		connected = false
	...
	if value != null:
		if has_node(value):
			get_node(value).connect("points_modified",Callable(self,"_handle_point_change"))
			connected = get_node(value).is_connected("points_modified",Callable(self,"_handle_point_change"))
			get_node(value).connect("tree_exiting",Callable(self,"_monitored_node_leaving"))
	...
	if monitored_shape != null:
		if has_node(monitored_shape) == true:
			...
			if node.is_queued_for_deletion() == true:
				node.disconnect("points_modified",Callable(self,"_handle_point_change"))
				return
			...
			var pt_a:Vector2 = node.global_transform * node.get_point_position(pt_a_index)
			var pt_b:Vector2 = node.global_transform * node.get_point_position(pt_b_index)
			...
			if (control_point_offset >= 0):
				pt_a_handle = node.global_transform * (node.get_point_position(pt_a_index) + node.get_point_out(pt_a_index))
				pt_b_handle = node.global_transform * (node.get_point_position(pt_b_index) + node.get_point_in(pt_b_index))
				...
				pt_a_handle = node.global_transform * (node.get_point_position(pt_a_index) + node.get_point_in(pt_a_index))
				pt_b_handle = node.global_transform * (node.get_point_position(pt_b_index) + node.get_point_out(pt_b_index))
