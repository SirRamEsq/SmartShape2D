...
@export var _constraints = {} : set = set_constraints
# Next key value to generate
@export var _next_key = 0 : set = set_next_key
...
func _ready():
	for tuple in _material_overrides:
		...
		if not mat.is_connected("changed",Callable(self,"_on_material_override_changed")):
			mat.connect("changed",Callable(self,"_on_material_override_changed").bind(tuple))
...
func set_points(ps: Dictionary):
	...
	for k in ps:
		var p = ps[k]
		p.connect("changed",Callable(self,"_on_point_changed").bind(p))
	_points = ps
...
func add_point(point: Vector2, idx: int = -1, use_key: int = -1) -> int:
	...
	new_point.connect("changed",Callable(self,"_on_point_changed").bind(new_point))
...
func remove_point(key: int) -> bool:
	if has_point(key):
		...
		var p = _points[key]
		if p.is_connected("changed",Callable(self,"_on_point_changed")):
			p.disconnect("changed",Callable(self,"_on_point_changed"))
...
func set_material_overrides(dict:Dictionary):
	...
	if _material_overrides != null:
		for old in _material_overrides.values():
			if old.is_connected("changed",Callable(self,"_on_material_override_changed")):
				old.disconnect("changed",Callable(self,"_on_material_override_changed"))

	_material_overrides = dict
	for tuple in _material_overrides:
		var m = _material_overrides[tuple]
		m.connect("changed",Callable(self,"_on_material_override_changed").bind(tuple))
