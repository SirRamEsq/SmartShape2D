...
func test_range_all_inclusive():
	var nr = RMSS2D_NormalRange.new(0.0, 360.0)
	var vectors = [
		Vector2(1000, 0),
		Vector2(1000, 1),
		Vector2(1000, 10),
	...
	nr = RMSS2D_NormalRange.new(0.0, 0.0)
	for i in range(0, vectors.size(), 1):
		var v = vectors[i]
		assert_true(nr.is_in_range(v), "[%s] Not In Range: %s" % [i, v])
...
func test_range():
	var nr = RMSS2D_NormalRange.new(-90.0, 90.0)
...
func test_angle():
	...
	var v1_angle = RMSS2D_NormalRange.get_angle_from_vector(v1)
	assert_eq(v1_angle, 0.0)

	# UP
	var v2 = Vector2(0, -1000)
	var v2_angle = RMSS2D_NormalRange.get_angle_from_vector(v2)
	assert_eq(v2_angle, 90.0)

	# LEFT
	var v3 = Vector2(-1000, 0)
	var v3_angle = RMSS2D_NormalRange.get_angle_from_vector(v3)
	assert_eq(v3_angle, 180.0)

	# DOWN
	var v4 = Vector2(0, 1000)
	var v4_angle = RMSS2D_NormalRange.get_angle_from_vector(v4)
