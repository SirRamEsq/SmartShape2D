...
func test_action_delete_point() -> void:
	...
	var a1 := ActionDeletePoint.new(s, pa.get_point_key_at_index(0))
	a1.do()
	assert_eq(pa.get_point_count(), 1)
...
func test_action_move_verticies() -> void:
	...
	var action := ActionMoveVerticies.new(s, pa.get_all_point_keys(), old_positions)
	action.do()
...
func test_action_open_shape() -> void:
	...
	var action := ActionOpenShape.new(s, pa.get_point_key_at_index(1))
	action.do()
...
func test_action_split_shape() -> void:
	...
	var action := ActionSplitShape.new(s, pa.get_point_key_at_index(1))
	action.do()
